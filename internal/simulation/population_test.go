...
func TestPopulationModel_SeedPopulation_SkipsNonPositiveFoodAndClearsPopulation(t *testing.T) {
	pm := simulation.NewPopulationModel(simulation.PopulationParameters{MaxCellCapacity: 10})
	tm := terrainMapWithFoodCapacities(-1, 0, 2)
	...
	if tm.Cells[0].Population != 0 || tm.Cells[1].Population != 0 {
		t.Fatalf(
			"expected non-positive food cells to remain empty, got [%.0f %.0f]",
			tm.Cells[0].Population,
			tm.Cells[1].Population,
		)
	}
